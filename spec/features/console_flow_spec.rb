...
if ENV['CF_V2_TEST_USER'] && ENV['CF_V2_TEST_PASSWORD'] && ENV['CF_V2_TEST_TARGET']
  describe 'A new user tries to use Rails Console against v2', :ruby19 => true do
    ...
    it 'pushes a simple rails app and consoles to it' do
      ...
      Dir.chdir("#{SPEC_ROOT}/assets/rails328_ruby187_app") do
        run("#{cf_bin} push") do |runner|
          ...
          expect(runner).to say /: postgresql/
          expect(runner).to say "What kind?>"
          runner.send_keys "postgresql"

          expect(runner).to say "Name?> postgresql-"
          runner.send_keys service
          ...
          # skip this
          if runner.expect "Bind other services to application?> n", 2
            runner.send_keys ""
          end
          ...
          expect(runner).to say /(Using|Installing) Ruby/i, 20
          expect(runner).to say "Your bundle is complete!", 60

          expect(runner).to say "Checking #{app}...", 120
          expect(runner).to say "1/1 instances"
          expect(runner).to say "OK", 30
        end

        run("#{cf_bin} console #{app}") do |runner|
          expect(runner).to say /Opening console on port .* OK/, 300
          expect(runner).to say /irb.*>/, 60

          runner.send_keys "1 + 1"
          expect(runner).to say "=> 2"

          runner.send_keys "exit"
        end
      end

      run("#{cf_bin} delete #{app} --delete-orphaned") do |runner|
