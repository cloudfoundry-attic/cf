module Luft
  module Helpers
    ...
    def confirm_billing
      ...
      display "For more information, see https://devcenter.luft.com/articles/usage-and-billing"
      if confirm
        Luft::Auth.client.confirm_billing
        true
      end
    end
    ...
    def confirm_command(app_to_confirm = app, message=nil)
      if confirmed_app = Luft::Command.current_options[:confirm]
        unless confirmed_app == app_to_confirm
          raise(Luft::Command::CommandFailed, "Confirmed app #{confirmed_app} did not match the selected app #{app_to_confirm}.")
        end
        return true
      else
        display
    ...
    def json_encode(object)
      Luft::OkJson.encode(object)
    rescue Luft::OkJson::Error
      nil
    end

    def json_decode(json)
      Luft::OkJson.decode(json)
    rescue Luft::OkJson::Error
      nil
    end
    ...
    def fail(message)
      raise Luft::Command::CommandFailed, message
    end
    ...
    def action(message, options={})
      display("#{message}... ", false)
      Luft::Helpers.error_with_failure = true
      ret = yield
      Luft::Helpers.error_with_failure = false
    ...
    def error(message)
      if Luft::Helpers.error_with_failure
        display("failed")
        Luft::Helpers.error_with_failure = false
      end
    ...
    def format_error(error, message='Luft client internal error.')
      formatted_error = []
      formatted_error << " !    #{message}"
      formatted_error << ' !    Search for help at: https://help.luft.com'
      formatted_error << ' !    Or report a bug at: https://github.com/luft/luft/issues/new'
      formatted_error << ''
      ...
      formatted_error << "    Version:     #{Luft.user_agent}"
      formatted_error << "\n"
      formatted_error.join("\n")
    end

    def styled_error(error, message='Luft client internal error.')
      if Luft::Helpers.error_with_failure
        display("failed")
        Luft::Helpers.error_with_failure = false
      end
