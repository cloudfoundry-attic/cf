...
require 'luft/client/ssl_endpoint'

# A Ruby class to call the Luft REST API.  You might use this if you want to
# manage your Luft apps from within a Ruby program, such as Capistrano.
#
# Example:
#
#   require 'luft'
#   luft = Luft::Client.new('me@example.com', 'mypass')
#   luft.create()
#
class Luft::Client
  ...
  def initialize(user, password, host=Luft::Auth.host)
    require 'rest_client'
    @user = user
    @password = password
    @host = host
  ...
  def self.deprecate
    ...
    $stderr.puts(" !    DEPRECATED: Luft::Client##{method} is deprecated, please use the heroku-api gem.")
    $stderr.puts(" !    DEPRECATED: More information available at https://github.com/luft/luft.rb")
  ...
  def self.auth(user, password, host=Luft::Auth.host)
  ...
  # Run a rake command on the Luft app and return output as a string
...
Check the output of "luft ps" and "luft logs" for more information.
    ERROR
  end
  ...
  def run_console_command(url, command, prefix=nil)
    ...
    if e.http_code == 422
      Luft::Command.extract_error(e.http_body, :raw => true)
    else
      raise e
    end
  end
  ...
  def process(method, uri, extra_headers={}, payload=nil)
    headers = luft_headers.merge(extra_headers)
  ...
  def extract_warning(response)
    ...
    if response.headers[:x_luft_warning] && @warning_callback
      warning = response.headers[:x_luft_warning]
      @displayed_warnings ||= {}
  ...
  def luft_headers # :nodoc:
    {
      'X-Heroku-API-Version' => '2',
      'User-Agent' => Luft.user_agent,
  ...
  def default_resource_options_for_uri(uri)
    if ENV["HEROKU_SSL_VERIFY"] == "disable"
      {}
    elsif realize_full_uri(uri) =~ %r|^https://api.luft.com|
