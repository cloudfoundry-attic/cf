...
#
class Luft::Command::Help < Luft::Command::Base
  ...
  #
  # $ luft help
  # Usage: luft COMMAND [--app APP] [command-specific-options]
  #
  # Primary help topics, type "luft help TOPIC" for more details:
  #
  ...
  # Additional topics:
  #
  #   account      #  manage luft account options
  #   accounts     #  manage multiple luft accounts
  #   ...
  #
  # $ luft help apps:create
  # Usage: luft apps:create [NAME]
  #
  #  create a new app
  #
  #      --addons ADDONS        # a comma-delimited list of addons to install
  #  -b, --buildpack BUILDPACK  # a buildpack url to use for this app
  #  -r, --remote REMOTE        # the git remote to create, default "luft"
  ...
  def commands_for_namespace(name)
    Luft::Command.commands.values.select do |command|
      command[:namespace] == name && command[:command] != name
    end
  end

  def namespaces
    namespaces = Luft::Command.namespaces
    namespaces.delete("app")
    namespaces
  end

  def commands
    Luft::Command.commands
  end

  def legacy_help_for_namespace(namespace)
    instance = Luft::Command::Help.groups.map do |group|
  ...
  def legacy_help_for_command(command)
    Luft::Command::Help.groups.each do |group|
  ...
  def help_for_root
    puts "Usage: luft COMMAND [--app APP] [command-specific-options]"
    puts
    puts "Primary help topics, type \"luft help TOPIC\" for more details:"
    puts
  ...
  def help_for_command(name)
    if command_alias = Luft::Command.command_aliases[name]
      display("Alias: #{name} redirects to #{command_alias}")
      name = command_alias
    end
    if command = commands[name]
      puts "Usage: luft #{command[:banner]}"
    ...
    if !namespace_commands.empty?
      puts "Additional commands, type \"luft help COMMAND\" for more details:"
      puts
      help_for_namespace(name)
      puts
    elsif command.nil?
      error "#{name} is not a luft command. See `luft help`."
    end
  end
end
