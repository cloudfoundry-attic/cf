...
module Luft
  module Command
    ...
    def self.prepare_run(cmd, args=[])
      ...
      begin
        usage_directory = "#{home_directory}/.luft/usage"
        FileUtils.mkdir_p(usage_directory)
        usage_file = usage_directory << "/#{Luft::VERSION}"
      ...
      if command
        ...
        error([
          "`#{cmd}` is not a luft command.",
          suggestion(cmd, commands.keys + command_aliases.keys),
          "See `luft help` for a list of available commands."
        ].compact.join("\n"))
      end
    end
    ...
    def self.run(cmd, arguments=[])
      ...
      error "API request timed out. Please try again, or contact support@luft.com if this issue persists."
      ...
      if e.message == 'getaddrinfo: nodename nor servname provided, or not known (SocketError)'
        error("Unable to connect to Luft API, please check internet connectivity and try again.")
      else
        raise(e)
      end
    ensure
    ...
    def self.extract_error(body, options={})
      default_error = block_given? ? yield : "Internal server error.\nRun `luft status` to check for known platform issues."
